...
type SpectatorV5ParticipantDto struct {
...
type SpectatorV5Dto struct {
	...
	Participants []SpectatorV5ParticipantDto `json:"participants"`
}

func GetSpectatorInfoV5(puuid string) (*SpectatorV5Dto, int, error) {
	riot.UpdateRiotApiCalls()
	resp, err := http.Get(http.GetRequest{
		Url: riot.CreateUrl(riot.RegionKr, "/lol/spectator/v5/active-games/by-summoner/"+puuid),
	})
	if err != nil {
		return nil, 0, err
	}
