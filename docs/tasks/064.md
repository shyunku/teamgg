...
- 숙련도: compact shadow 구현, 격리 MySQL 100만 행 copy 검증과 10만 행 sync trigger/read cutover 검증 완료, 운영 maintenance cutover 미실행
- 운영 API: strict scan 핫픽스 `f271d60` 배포 완료, backend·replay healthy 및 주요 API 200
- 디스크: 최근 확인 기준 128GiB 중 약 27GiB 여유, 사용률 80%
- 예상 시간: 숙련도 shadow copy+검증의 로컬 단순 환산은 약 62분이며 운영은 I/O·binlog·안전 점검을 포함해 별도 maintenance window로 잡는다.
