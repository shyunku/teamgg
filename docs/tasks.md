...
- 구현은 끝났지만 운영 환경·실제 데이터·장기 관측 등의 검증이 남았다면 `🟣 VFY`로 변경하고 미검증 항목을 명시합니다.
- `VFY` 중 코드 수정이 다시 필요하면 `🟡 WIP`로 되돌립니다.
- 구현과 검증이 모두 끝난 작업만 `🟢 DONE`으로 변경합니다.
- 동시에 실제 코드 구현 묶음 하나만 `WIP`로 두되, 배포·운영 규모 검증 대기 항목은 병행할 수 있습니다.
- `Tag`는 `backend`, `frontend`, `replay`, `unclassified` 중 하나만 사용합니다.
- `Date`는 TODO/WIP/VFY의 등록 또는 갱신일, DONE의 완료일을 `YYYY-MM-DD`로 기록합니다.
...
`#76, #70, #71, #72, #65, #75, #42, #51, #50, #52, #49, #44, #45, #46, #47, #48`
...
| 76 | backend | 🟣 VFY | 2026-09-24 17:41 | #66 | 오래된 경기 자동 정리 | 최신 8개 패치를 보존하는 7일 간격 systemd timer를 운영에 활성화했다. dry-run·중복 실행 방지·백엔드 복구·디스크/DB 부하 제한과 로컬·운영 no-op 실행을 검증했다. 실제 만료 경기 삭제 및 알림 웹훅은 아직 검증되지 않아 VFY로 추적한다. |
...
| 66 | backend | 🟢 DONE | 2026-09-02 22:34 | #58, #59 | 오래된 경기 데이터 정리 | 최신 8개 패치를 보존하고 숫자 identity를 유지하는 bounded delete를 구현했다. 운영에서 48,351경기와 종속 상세 데이터를 정리했으며 최종 dry-run 잔여 0건, backend healthy, 주요 API 200을 확인했다. |
