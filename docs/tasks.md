...
`#67, #61, #62, #63, #64, #65, #66, #42, #51, #50, #52, #49, #44, #45, #46, #47, #48`

## 작업 목록

| Index | Tag | Status | Date | Dependencies | Description |
|---:|---|---|---|---|---|
| 67 | backend | 🔴 TODO | 2026-08-20 | — | DataExplorer 일일 처리량, 신규 소환사·경기·숙련도·큐 행 증가량과 DB 디스크·임시 공간 사용량을 구조화 로그/메트릭으로 노출하고 임계치 경고 및 운영 권장 예산을 문서화한다. |
...
| 62 | backend | 🔴 TODO | 2026-08-20 | — | Champion Detail/메타 통계가 전체 데이터를 여러 임시 테이블로 복제·정렬하지 않도록 최근 패치 선필터, 적절한 영구 집계 테이블 또는 증분 집계로 재설계하고 실행 시간·임시 공간 사용량을 전후 비교한다. |
| 61 | backend | 🔴 TODO | 2026-08-20 | — | 2,900만 건 전체를 장시간 스캔하는 숙련도 통계를 커버링 인덱스 또는 증분 집계로 개선하고, 운영 규모 데이터에서 실행 시간·잠금·쿼리 계획을 검증한 뒤 기존 12시간 전체 집계를 교체한다. |
