...
func applyIncrementalChampionDetailStatistics(ctx context.Context, database *sqlx.DB) error {
	statements := []string{
		`CREATE TABLE IF NOT EXISTS champion_detail_statistics_participants (
			...
			damage_self_mitigated INT NOT NULL, damage_dealt_to_buildings INT NOT NULL,
			damage_dealt_to_objectives INT NOT NULL, damage_dealt_to_turrets INT NOT NULL,
			enemy_champion_id INT NULL, enemy_champion_name VARCHAR(255) NULL,
			enemy_kills INT NULL, enemy_deaths INT NULL, enemy_assists INT NULL, enemy_win TINYINT(1) NULL,
			build_valid TINYINT(1) NOT NULL DEFAULT 0,
			primary_style INT NULL, primary_perk0 INT NULL, primary_perk1 INT NULL,
			primary_perk2 INT NULL, primary_perk3 INT NULL,
			sub_style INT NULL, sub_perk0 INT NULL, sub_perk1 INT NULL,
			stat_perk_defense INT NULL, stat_perk_flex INT NULL, stat_perk_offense INT NULL,
		...
		`CREATE OR REPLACE VIEW champion_detail_statistics_valid_builds AS
		 SELECT * FROM champion_detail_statistics_participants WHERE build_valid = 1`,
	}
...
func validateIncrementalChampionDetailStatistics(ctx context.Context, database *sqlx.DB) (bool, error) {
	columns, err := columnsExist(ctx, database, map[string][]string{
		"champion_detail_statistics_participants": {
			"source_id", "game_version", "match_id", "match_participant_id", "game_duration",
			"champion_id", "team_position", "build_valid", "primary_style", "item0_id",
			"total_minions_killed", "damage_self_mitigated",
		},
		...
		"champion_detail_statistics_processed_matches": {
			"game_version", "match_id", "processed_at",
		},
		"champion_detail_statistics_progress": {
			"game_version", "last_match_id", "processed_matches", "completed", "updated_at",
		},
	})
	if err != nil || !columns {
		return false, err
	}
	views, err := tablesExist(ctx, database, "champion_detail_statistics_valid_builds")
	if err != nil || !views {
		return false, err
	}
	indexes := []struct {
		...
		{"champion_detail_statistics_participants", "PRIMARY", []string{"source_id"}},
		{"champion_detail_statistics_participants", "champion_detail_participant_uindex", []string{"match_participant_id"}},
		{"champion_detail_statistics_participants", "champion_detail_participant_version_match_index", []string{"game_version", "match_id"}},
		{"champion_detail_statistics_participants", "champion_detail_participant_match_index", []string{"match_id"}},
		{"champion_detail_statistics_participants", "champion_detail_participant_meta_index", []string{"build_valid", "champion_id", "team_position", "primary_style", "sub_style"}},
		{"champion_detail_statistics_participants", "champion_detail_participant_counter_index", []string{"build_valid", "champion_id", "enemy_champion_id", "team_position"}},
		{"champion_detail_statistics_bans", "PRIMARY", []string{"match_id", "team_id", "pick_turn"}},
		{"champion_detail_statistics_bans", "champion_detail_bans_version_champion_index", []string{"game_version", "champion_id"}},
		{"champion_detail_statistics_processed_matches", "PRIMARY", []string{"game_version", "match_id"}},
		{"champion_detail_statistics_progress", "PRIMARY", []string{"game_version"}},
	}
