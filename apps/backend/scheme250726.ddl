...
create index match_participant_perk_styles_description_index
    on match_participant_perk_styles (description);
...
create index match_participants_champion_id_index
    on match_participants (champion_id);
...
create index match_participants_team_position_index
    on match_participants (team_position);
...
create index tag
    on static_item_tags (tag);

create index depth
    on static_items (depth desc);

create index gold_total
    on static_items (gold_total desc);

create index name
    on static_items (name);
...
create table custom_game_participant_color_labels
(
    custom_game_config_id varchar(255) not null,
    puuid                 varchar(255) not null,
